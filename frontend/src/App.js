...
function App() {
  ...
  useEffect(() => {
    if (searchQuery.length >= 2) {
      axios.get(`http://localhost:5000/api/search/positions?query=${searchQuery}`)
  ...
  const fetchData = () => {
    ...
    Promise.all([
      axios.get(`http://localhost:5000/api/salary-data?${params}`),
      axios.get(`http://localhost:5000/api/grade-stats?${params}`)
    ])
